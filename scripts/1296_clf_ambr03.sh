...
INPUT_DIR="$HOME/tar_ball/ABF_FA_AMBR01-NSR"
...
lmcontrol opta-clf-train \
    ...
    --val_frac 0.2 \
    --seed 42 \
    --checkpoint $HOME/output/optatune/ambr03/opta_${task_name}/ \
    --epochs 5 \
    --outdir $HOME/output/optatune/ambr03/opta_${task_name}/ \
    -n 95 \
    --early_stopping \
    testing0 \
    feed

best_ckpt=$(ls $HOME/output/optatune/ambr03/opta_${task_name}/ | \
    grep 'checkpoint-' | \
    sort -t '-' -k3,3 -g | \
    tail -n 1)

best_ckpt_path="$HOME/output/optatune/ambr03/opta_${task_name}/$best_ckpt"
...
lmcontrol opta-clf-predict \
    ...
    -o $HOME/output/ambr03/prediction_${task_name}.npz \
    -n 95 \
    feed \
